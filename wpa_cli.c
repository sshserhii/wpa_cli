...
#pragma pack(1) // packing our struct to send it via socket

struct network_connection_str{
	int network_id;
	int checksum;
	char ssid[20];
	char identity[40];
...
static int _wpa_ctrl_command(struct wpa_ctrl *ctrl, char *cmd, int print)
{
	...
	if (print) {
		buf[len] = '\0';
		printf("%s", buf);
    strncpy(BufferSend, buf, sizeof(buf));
...
static void wpa_cli_get_result_struct(struct wpa_supplicant *wpa_s,struct scan_info *info, int new_scan){
	struct wpa_scan_results* results;
	results = wpa_supplicant_get_scan_results(wpa_s,NULL,0);


}
static void wpa_server_packet_checksum(){
        int CheckSum = 0;
      //  while(size > 0){
             //   CheckSum+= *buffer;
              //  buffer+=1;
               // size--;
      //  }
        CheckSum = (CheckSum >> 16) + (CheckSum & htonl(0x0000ffff));
       // std::cout<<CheckSum;
...
void deserialize(const char* buffer){
	...
	memcpy(temp_connection.ssid,buffer + offset,sizeof(temp_connection.ssid));
	offset += sizeof(temp_connection.ssid);

	memcpy(temp_connection.identity,buffer + offset,sizeof(temp_connection.identity));
	offset += sizeof(temp_connection.identity);

	memcpy(temp_connection.password,buffer + offset,sizeof(temp_connection.password));
	offset += sizeof(temp_connection.password);

	memcpy(temp_connection.key_mgmt,buffer + offset,sizeof(temp_connection.key_mgmt));
	offset += sizeof(temp_connection.key_mgmt);

}

void status(){
	printf("%s\n", temp_connection.ssid);
	printf("%s\n", temp_connection.identity);
	printf("%s\n", temp_connection.password);
	printf("%s\n", temp_connection.key_mgmt);
}
void connect_to_network(struct wpa_ctrl *ctrl){
	wpa_ctrl_command(ctrl, "ADD_NETWORK");
	temp_connection.network_id =atoi(BufferSend);
	int size =3;
	char* array[size];

	array[0]=BufferSend;
	array[1]="ssid";
	array[2]= temp_connection.ssid;
	wpa_cli_cmd(ctrl, "SET_NETWORK", 3, size, array);

	array[0]=BufferSend;
	array[1]="identity";
	array[2]= temp_connection.identity;
	wpa_cli_cmd(ctrl, "SET_NETWORK", 3, size, array);

	array[0]=BufferSend;
	array[1]="password";
	array[2]= temp_connection.password;
	wpa_cli_cmd(ctrl, "SET_NETWORK", 3, size, array);

	array[0]=BufferSend;
	array[1]="key_mgmt";
	array[2]= temp_connection.key_mgmt;
	wpa_cli_cmd(ctrl, "SET_NETWORK", 3, size, array);

	array[0]=BufferSend;
	wpa_cli_cmd(ctrl, "ENABLE_NETWORK", 1, 1, array);

}
static void wpa_cli_test_server_start(struct wpa_ctrl *ctrl, int argc, char *argv[]){
	...
	int length_of_send_data=4096;
	char struct_send_buffer[256];
	memset(buff, 0, 4096);
          ...
          afd = accept(sfds[i], (struct sockaddr *)&sock[i], &len);
					...
					char * p=buff;
						do{
								...
								while(length_of_send_data>0 && (len = recv(afd, p, sizeof(buff),MSG_NOSIGNAL)))
								{
									length_of_send_data-=len;
									p+=len;

								}

							p=&buff[0];
							length_of_send_data=4096;
						...
						if(len>0){
							...
							else if (strcmp(buff,"scan_results")==0){

								wpa_ctrl_command(ctrl, "SCAN_RESULTS");

						} else if (strcmp(buff,"send_struct")==0){

								len = recv(afd, struct_send_buffer, sizeof(struct_send_buffer),MSG_NOSIGNAL);
								deserialize(struct_send_buffer);
								status();

						} else if(strcmp(buff,"connect ")){

								connect_to_network(ctrl);

						}
							else{

								printf("command not enable");

						}

						if(BufferSend>0){
								while(length_of_send_data>0 && (len = send(afd, BufferSend, 4096,MSG_NOSIGNAL))>0){
										length_of_send_data -=len;
										p+=len;
									}
									p=&buff[0];
...
memset(&BufferSend[0], 0, sizeof(BufferSend));
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
	...
	{ "server", wpa_cli_test_server_start, NULL,
	  cli_cmd_flag_none,
	  "= start's server for android client"},
	...
	{ "help", wpa_cli_cmd_help, wpa_cli_complete_help,
	  cli_cmd_flag_none,wpa_cli_cmd_enable_network,
